...
#        _ = rp._check_rainfall_rate_values(rain_source, times_in_1st_col=True)
    ...
    def get_time_series(self, method='mean'):
    ...
    def get_attrs(self):
        """ define rain_attr
        """
        _, mask_valid = self.get_valid_rain_rate()
        time_s = self.time_s
        ...
        spatial_res = np.sqrt(np.median(source_counts))*cellsize
        cell_mean = self.get_time_series(method='mean') # mm/h
        # total value along time axis for a cell
        rain_total = np.trapz(cell_mean[:,1], x=time_s/3600, axis=0) # mm
        rain_mean = rain_total/(time_s.max()-time_s.min())*3600 # mm/h
        cell_max = self.get_time_series(method='max')
        attrs = {}
        attrs['num_source'] = num_source
        attrs['max'] = [np.max(cell_max[:,1]).round(2), 'mm/h']
